...
public class CocosVideoView extends SurfaceView implements IVideoPlayerView {
    ...
    public void setVideoFileName(String path) {
        ...
        if (path.startsWith("/")) {
            ...
            if (mVideoFilePath != null && !mVideoFilePath.isEmpty()) {
                reload = path.compareTo(mVideoFilePath) != 0 ? true : false;
            }
            mVideoFilePath = path;
...
//            mMediaPlayer.prepareAsync();
    ...
    MediaPlayer.OnPreparedListener mPreparedListener = new MediaPlayer.OnPreparedListener() {
        public void onPrepared(MediaPlayer mp) {
            ...
            if(!mMetaUpdated) {
                /// 替换资源需要再次发送meta_loaded事件，修改适配层_loadedMeta标识
                Log.d("[Mocos]", "sendMetaLoaded event");
...
//                    Log.d("[Mocos]", " mediaplayer play completed");
                    mCurrentState = State.PLAYBACK_COMPLETED;
